...
/// t: time to maturity
pub fn price(s: f64, k: f64, r: f64, sigma: f64, t: f64, greeks: &mut Option<Greeks>) -> f64 {
    let sqrt_t = t.sqrt();
...
pub fn implied_volatility_bissection(
    ...
    if price_ > option_price {
        return Ok(0.0);
    }

    let mut sigma_high = 0.3f64;
    price_ = price(s, k, r, sigma_high, t, &mut no_greek);
    while price_ < option_price {
        sigma_high *= 2.0;
        price_ = price(s, k, r, sigma_high, t, &mut no_greek);
    ...
    for _ in 0..MAX_ITERATIONS {
        ...
        price_ = price(s, k, r, sigma, t, &mut no_greek);
        let diff = price_ - option_price;
